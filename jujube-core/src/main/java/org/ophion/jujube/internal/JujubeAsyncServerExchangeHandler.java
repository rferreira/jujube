...
public class JujubeAsyncServerExchangeHandler extends AbstractServerExchangeHandler<Message<HttpRequest, HttpEntity>> {
  ...
  public JujubeAsyncServerExchangeHandler(JujubeConfig config, Function<JujubeHttpContext, JujubeHttpResponse> handler) {
    this.config = config;
  ...
  protected AsyncRequestConsumer<Message<HttpRequest, HttpEntity>> supplyConsumer(HttpRequest request, EntityDetails entityDetails, HttpContext context) throws HttpException {
    return new ContentAwareRequestConsumer<>(config, entityDetails, new AtomicReference<>());
  }
